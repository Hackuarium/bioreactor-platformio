...
#define PARAM_SEDIMENTATION_TIME \
  31  // AF - MINUTES to wait without rotation before emptying
#define PARAM_FILLED_TIME 32  // AG - MINUTES to stay in the filled state
#define PARAM_WEIGHT_FACTOR \
  33  // AH - Weight calibration: conversion factor digital -> gr
      // (weight=FACTOR*dig_unit)
#define PARAM_WEIGHT_OFFSET \
  34  // AI - Weight calibration: digital offset value when bioreactor is empty
...
#define FLAG_TEMP_LIQ_PROBE_ERROR \
  1  // liquid probe failed (one wire not answering)
