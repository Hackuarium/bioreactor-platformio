...
#define PID_OUTPUT_LIMIT \
  200  // 200 is ~80% of max PWM --> Limits max avg power to ~8A
...
void pid_ctrl() {
  saveAndLogError(getParameter(PARAM_TEMP_LIQ) < SAFETY_MIN_LIQ_TEMP ||
                      getParameter(PARAM_TEMP_LIQ) > SAFETY_MAX_LIQ_TEMP,
                  FLAG_TEMP_LIQ_RANGE_ERROR);
  ...
  if (isRunning(FLAG_SEDIMENTATION) ||
      isRunning(FLAG_RELAY_EMPTYING)) {  // when it is in sedimentation or
                                         // emptying mode we stop the heating
    analogWrite(PID_CONTROL, 0);
    return;
  }
