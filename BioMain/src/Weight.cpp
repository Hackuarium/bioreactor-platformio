...
NIL_THREAD(ThreadWeight, arg) {
  nilThdSleepMilliseconds(
      1234);  // wait a little bit not everything starts at once
  ...
  ********************************************/
  ...
  long timeLastEvent =
      millis() - getParameter(PARAM_WAIT_SINCE_LAST_EVENT) * 60000;
  ...
  ********************************************/
  while (true) {
    ...
    int error =
        (getParameter(PARAM_WEIGHT_MAX) - getParameter(PARAM_WEIGHT_MIN)) / 5;

    if ((error > 0 && ((weight < (getParameter(PARAM_WEIGHT_MIN) - error)) ||
                       (weight > (getParameter(PARAM_WEIGHT_MAX) + error)))) ||
        (error < 0 && ((weight > (getParameter(PARAM_WEIGHT_MIN) - error)) ||
                       (weight < (getParameter(PARAM_WEIGHT_MAX) + error))))) {
    ...
    if (isRunning(FLAG_SEDIMENTATION)) {
      ...
      if (sinceLastEvent >=
          getParameter(PARAM_SEDIMENTATION_TIME)) {  // switch to Emptying
      ...
      if ((error > 0 && weight <= getParameter(PARAM_WEIGHT_MIN)) ||
          (error < 0 &&
           weight >= getParameter(PARAM_WEIGHT_MIN))) {  // switch fo Filling
        stopProcess(FLAG_RELAY_EMPTYING);
      ...
      if (sinceLastEvent >=
          getParameter(PARAM_FILLED_TIME)) {  // switch to Sedimentation
...
int convertWeightToG(int weight) {
  return ((long)(weight - getParameter(PARAM_WEIGHT_OFFSET)) * 1000) /
  ...
  switch (command) {
    ...
    case 'k':  // define the weight one kg over the tare
      setAndSaveParameter(PARAM_WEIGHT_FACTOR,
                          weight - getParameter(PARAM_WEIGHT_OFFSET));
      output->println(weight);
      break;
