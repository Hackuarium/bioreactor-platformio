...
#define PID_OUTPUT_LIMIT \
  100  // 200 is ~80% of max PWM --> Limits max avg power to ~8A
...
void pid_ctrl() {
  saveAndLogError(getParameter(PARAM_TEMP_EXT1) < SAFETY_MIN_LIQ_TEMP ||
                      getParameter(PARAM_TEMP_EXT1) > SAFETY_MAX_LIQ_TEMP,
                  FLAG_TEMP_EXT1_RANGE_ERROR);
  saveAndLogError(getParameter(PARAM_TEMP_EXT2) < SAFETY_MIN_LIQ_TEMP ||
                      getParameter(PARAM_TEMP_EXT2) > SAFETY_MAX_LIQ_TEMP,
                  FLAG_TEMP_EXT2_RANGE_ERROR);
  saveAndLogError(getParameter(PARAM_TEMP_PCB) < SAFETY_MIN_PCB_TEMP ||
                      getParameter(PARAM_TEMP_PCB) > SAFETY_MAX_PCB_TEMP,
                  FLAG_TEMP_PCB_RANGE_ERROR);
  saveAndLogError(getParameter(PARAM_TEMP_TARGET) < SAFETY_MIN_LIQ_TEMP ||
                      getParameter(PARAM_TEMP_TARGET) > SAFETY_MAX_LIQ_TEMP,
                  FLAG_TEMP_TARGET_RANGE_ERROR);

  if (!isRunning(FLAG_PID_CONTROL) ||
      !isEnabled(FLAG_PID_CONTROL)) {  // PID is disabled
...
THD_FUNCTION(ThreadPID, arg) {
  ...
  while (true) {
    pid_ctrl();
    chThdSleep(
        500);  // refresh every 500ms --> the faster the better the control
  }
}
